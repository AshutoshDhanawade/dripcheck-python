...
from bundle_generate.models import FootwearAiRecommendation


class Command(BaseCommand):
    help = 'Import footwear AI recommendations from CSV file'
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(f'Clearing existing footwear AI recommendations...')
        FootwearAiRecommendation.objects.all().delete()

        with open(csv_path, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            total = 0
            for row in reader:
                FootwearAiRecommendation.objects.create(
                    item_id=row['item_id'],
                    name=row['name'],
        ...
        self.stdout.write(self.style.SUCCESS(f'Successfully imported {total} footwear AI recommendations.'))
