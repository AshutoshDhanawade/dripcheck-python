...
from bundle_generate.models import BottomwearAiGeneration


class Command(BaseCommand):
    help = 'Import bottomwear AI generations from CSV file'
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write('Clearing existing bottomwear AI generations...')
        BottomwearAiGeneration.objects.all().delete()

        batch = []
        with open(csv_path, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                batch.append(BottomwearAiGeneration(
                    item_id=row['item_id'],
                    name=row['name'],
        ...
        BottomwearAiGeneration.objects.bulk_create(batch, batch_size=500)
        self.stdout.write(self.style.SUCCESS(f'Successfully imported {len(batch)} bottomwear AI generations.'))
